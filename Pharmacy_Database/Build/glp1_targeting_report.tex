...
Loss-per-fill economics (\$46--\$56/fill) are NADAC-weighted by state drug mix, computed from CMS NADAC (National Average Drug Acquisition Cost) data and Part D prescribing volume. National weighted average: \$52/fill. The \$275/month subscription price and 5\% moderate routing rate come from the RetailMyMeds GLP-1 Value Proposition.
...
\midrule
GLP-1 Loss & 30,808 (73.7\%) & ``You're losing \$46--\$56 on every GLP-1 fill. RetailMyMeds routes those prescriptions so you stop absorbing the loss.'' \\
...
\midrule
Low (dulaglutide-heavy states) & \$46/fill & NADAC-weighted, MA/WI/UT drug mix \\
National average & \$52/fill & NADAC-weighted, national drug mix \\
High (semaglutide-heavy states) & \$56/fill & NADAC-weighted, AK/ND/SD drug mix \\
\bottomrule
\end{tabular}
...
\item \textbf{The financial model is per-pharmacy, not per-state.} Each row shows estimated fills, estimated losses, projected savings at \$275/month, and estimated ROI. Fill estimates are exposure-weighted by prescriber proximity (CMS Part D data). Loss per fill is NADAC-weighted by state drug mix (\$46--\$56/fill). Higher-scoring pharmacies project higher because their area data suggests above-average GLP-1 volume.
...
\textbf{What is estimated:} Per-pharmacy GLP-1 fill volume (exposure-weighted from state CMS totals using prescriber proximity from Part D data), monthly loss (fills $\times$ NADAC-weighted loss/fill, \$46--\$56 by state drug mix), savings (assumes 5\% routing rate from Value Proposition), and ROI (savings / \$3,300 annual subscription).
